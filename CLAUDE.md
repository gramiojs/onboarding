...
The full design spec lives at `docs/SPEC.md`. Read it before making non-trivial changes.
...
- **Phase 5** — cross-chat scoping via `renderIn: "dm" | "group" | "any" | (ctx) => boolean`. When the step's `renderIn` constraint is incompatible with the current ctx's scope, `renderStep` returns `{ pending: true }` and the runner stashes `pendingStepId` on the record instead of rendering. The derive hook re-attempts rendering on every inbound update whose ctx becomes eligible for the pending step. `nextImpl` does not auto-complete the flow when the render of a terminal step was deferred — completion happens once the pending re-render actually succeeds (or never, if the step has its own terminal trigger). DM steps default to `{ next, skip, exit, dismiss }` controls; group steps default to `{ exit }` only (see `GROUP_DEFAULTS`).
- **Phase 6** — opt-out layer. Per-flow `dismiss()` / `undismiss()` / `isDismissed` with `onDismiss` hook; namespace-wide `disableAll()` / `enableAll()` / `allDisabled` / `exitAll()`. `startImpl` checks `globalLocal.disabled` first (`"opted-out"`), then per-flow `"dismissed"` status. The `exitAll` callback op routes to `runExitAll` which dismisses active/paused flows and flips `disableAll` for every registered storage.
- **Phase 7** — storage contract. The `storage:` option accepts any `Storage<OnboardingStorageMap>`, so third-party adapters from the `@gramio/storage-*` ecosystem (redis, sqlite, etc.) plug in without a wrapper. `getStorageContractCases(make)` is exported as a framework-agnostic suite (returns `{ name, run }[]`) that any adapter can wire into its own test runner to prove conformance. `memoryStorage()` passes it as a baseline regression check.

Future phases (8) are described in the spec.
